...
<div class="container">
    <form action="admin.php" method="post" enctype="multipart/form-data" class="py-4" id="myform">
        <div class="form-group">
            ...
            <input type="text" class="form-control validate[required]" name="title" value="{$article.title}" id="title" placeholder="請輸入文章標題">
        </div>

        <div class="form-group">
            <label for="content" class="col-form-label sr-only">文章內容</label>
            <textarea name="content" id="content" class="form-control validate[required, minSize[10]]" rows="10" placeholder="請輸入文章內容">{$article.content}</textarea>
        </div>
